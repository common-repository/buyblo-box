...
namespace Nokaut\BuybloApiKit\ClientApi\Rest;
...
use Nokaut\BuybloApiKit\ClientApi\Rest\Exception\InvalidRequestException;
use Nokaut\BuybloApiKit\ClientApi\Rest\Exception\NotFoundException;
...
class RestClientApi implements ClientApiInterface
{
    ...
    public function __construct(LoggerInterface $logger, EventSubscriberInterface $auth, $headers = array())
    {
    ...
    protected function sendMultiProcess(Fetches $fetches)
    {
        ...
        foreach ($fetches as $fetch) {
            ...
            $requests[] = $this->getClient()->createRequest(
    ...
    protected function handleException(BadResponseException $e, $startTime)
    {
        ...
        if ($statusCode == 404 || $statusCode == 400) {
            $exceptionMessage = (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '') . " " . $statusCode . " from api for request: "
                . $request->getUrl() . ', response body ' . $response->getBody(true);
        } else {
            $exceptionMessage = (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '') . " bad response from api (status: " . $statusCode . ") "
                . "for request: " . $request->getUrl() . ', response body: ' . $response->getBody(true);
        }
    ...
    protected function log($request, $response, $startTime, $level = LogLevel::DEBUG, $additionalMessage = '')
    {
        ...
        $apiStatusMessage = 'request to API: ' . $url . ' ';
        $apiStatusMessage .= $response ? 'status from API: ' . $response->getStatusCode() . ', body: ' . $response->getBody() : 'empty response ';
